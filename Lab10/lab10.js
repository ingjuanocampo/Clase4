...
function Validate(){};
  ...
  Validate.prototype = {
    email : function(email){///Valite esta herandando todo prototype
      var pattern = new RegExp(/^[\w-]+(\.[\w-]+)*@([\w-]+\.)+[a-zA-Z]+$/);
      return pattern.test(email);
    },
    userid : function(uid){//Valide se le esta agregando userId
      var pattern = new RegExp(/^[0-9]+$/);
      return pattern.test(uid);
    }
  };

  $('.nid-error').addClass('error');//selector de clase relacion con class en index.html, 
                                    //agragar clase definida en el style, cambiando asi la que tenia asociada desde el inicio del index.html
  $('.nid-error').hide();
  ...
  $('#buttons #info').remove();
    ...
    $('.button').bind('click', function(event){//Toma el id del html como el selector e intancia un call back para manejar el evento de click

    $("#info").remove();  


    var validate = new Validate();
    var data = $('#nid').val();//==var data = document.getElementById('nid').value; 

  if(validate.userid(data)){
      $('#nid').next().hide();
      $('.nid-error').next().hide();
      localStorage.setItem('nid', data);//localStorage es una tecnologia de HTML5 que permite alamancear informacion del lado del cliente, solo guarda STRING, maximo 2MB
    }
    else{
      $('#nid').next().show();
 ...
 var count=0, meals={};
    ...
    $('#checkboxes').find(':checkbox').each(function(){
      if($(this).is(':checked')){//If en este elementos esta chekeados
        count+=parseInt($(this).val());
        meals[$(this).attr('name')] = parseInt($(this).val());//Agregando un elemento al arreglo
      }
    });
    
    if(count==0){
      $('.food-error').css({'margin':50}).show();//Forma de agregar css a un elemento especifico
    ...
    event.preventDefault();//Detener cualquier mecanimos que envia al servidor ... (SUBMIT)
  });
